...
import { knowledgeEvalSummary, runKnowledgeEval } from "../../../lib/knowledgeEval";
...
export async function runInlineEvalAsync(kind: "policy" | "router" | "knowledge" = "policy"): Promise<InlineEvalView> {
  const preferServer = peekRuntimeConfig().features.preferServerEval;

  if (kind === "knowledge") {
    const rows = runKnowledgeEval();
    const sum = knowledgeEvalSummary(rows);
    return toInlineView(
      "knowledge",
      rows.map((r) => ({
        id: r.id,
...
/** 同步回退（无后端时） */
export function runInlineEval(kind: "policy" | "router" | "knowledge" = "policy"): InlineEvalView {
  if (kind === "knowledge") {
    const rows = runKnowledgeEval();
    const sum = knowledgeEvalSummary(rows);
    return toInlineView(
...
export function InlineEvalCard({ eval: data }: { eval: InlineEvalView }) {
  return (
    <div className={`ua-eval ${data.accuracy >= 80 ? "ok" : "warn"}`}>
      <header>
        <strong>
          {data.kind === "policy" ? "制度评测" : data.kind === "knowledge" ? "资料库命中评测" : "路由评测"}
        </strong>
        <span>

...
export const resumeProjectEntries: ResumeProjectEntry[] = [
  ...
  },
  {
    ...
    personal: true,
    workSlug: "ownagent",
    stack: ["React 19", "TypeScript", "Agent Loop", "MCP", "RAG", "能力锁", "SQLite"],
    sections: [
      {
        heading: "项目背景",
        paragraphs: [
          ...
          "产品形态以能力全景为底图。一句话经过意图路由 → MCP 工具 → 检索带 chunkId → 能力锁。对话、运行追踪、回归评测是同一条链上的切片，不是互不相关的实验室。问句先锁 read / mutate / abstain：制度问答只输出带条款编号的原句，开通 VPN 只起草工单，人点允许才提交。无后端时走浏览器内 Guest Runtime，有 Hono + SQLite 时走服务端 Loop，协议层保持一致。",
        ],
      },
      {
        heading: "核心模块",
        subsections: [
          ...
          },
          {
            heading: "能力锁 · 人机确认",
            bullets: [
              "问句先锁能力信封：只读制度、只许起草工单、手册外直接拒绝，检索分数不能把读抬成改权限",
              "制度答案只 emit 手册原句并绑定条款编号；现行 10 天和废止 5 天同时出现则冲突熔断，不合成一句",
              "开通 VPN / 改权限走 ticket_draft 预演，ticket_commit 在未 allow 时拒绝；评测同时卡信封、结果码、对话里误开通次数",
            ],
          },
          {
            heading: "分块 RAG",
          ...
          },
          {
            ...
            bullets: [
              ...
              "Eval 用固定 query → expectedSkillId 回归路由，另有能力锁用例卡 read/mutate/abstain 与结果码；工具侧记真实耗时",
    ...
    achievements: [
      ...
      "能力信封 + 工单预演：制度问答带出处，改权限对话里误开通为 0",
    ],
    plain: {
      name: "OwnAgent — 浏览器内 AI Agent 平台",
      role: "独立设计与开发",
      ...
      description:
        "独立设计并实现浏览器内 AI Agent 平台 OwnAgent。常见做法是把 Loop、工具协议、检索封在服务端 SDK，前端只消费一段 SSE；出错时 Network 只有一条 pending 长请求，分不清是意图路由选错、工具入参还是半截 JSON、还是检索根本没召回。本项目把这几层在 TypeScript 里拆开实现，不依赖 LangChain / LlamaIndex。能力全景是底图：一句话经过意图路由、MCP、检索、能力锁。对话、追踪、评测是同一条链上的切片。问句先锁 read / mutate / abstain：制度问答只输出带条款编号的原句，开通 VPN 只起草工单，人点允许才提交。无后端走浏览器内 Guest Runtime，有服务时走 Hono + SQLite，协议层保持一致。\n\n一、Agent Loop。模型以 SSE 增量下发 tool_call。同一 index 的 name / arguments 会拆成多帧，实现上按 index 做累积缓冲区，流结束或该 call 完整后再 JSON.parse，禁止半截参数进执行层。单轮最多 8 次「模型 → 工具 → 再模型」，超出直接停，防止工具互相回调死循环。工具结果写回 messages 再请求下一轮，直到模型不再发 tool_call 或达到上限。\n\n二、进程内 MCP。JSON-RPC 2.0 实现 tools/list 与 tools/call，不走独立进程、不依赖外部 MCP 宿主。每个工具带 JSON Schema，入参校验失败不进 execute。运行时异常捕获后打成 isError 结构回流给模型，让它改参或换工具；单工具失败不中断整轮，避免一次调用错误导致整页白屏。\n\n三、Skill 路由。技能用 SKILL.md 声明 name、triggers、tools、steps。用户问句分词后对每条技能的 trigger 加权：长词 2 分、短词 1 分，取 Top-1。全员零分则停住，不默认落到「站点体检」一类兜底技能——否则所有问题都会走同一条链，路由等于没做。explainDiscovery 返回完整打分矩阵，改 trigger 时用固定用例集跑命中率并输出失败样本，避免修 A 伤 B。\n\n四、分块 RAG。文档按段落切块，检索叠关键词重叠、项目名称直匹配、段落位置加权。命中结果带 chunkId，界面能回到原文，避免「搜到了但说不清哪一段」。无向量库依赖，浏览器内可跑。\n\n五、能力锁与人机确认。问句进 Loop 前先锁能力信封 read / mutate / abstain，检索分数不能把「只读制度」抬成「改权限」。制度问答只输出手册原句并绑定条款编号；现行 10 天和废止 5 天同时被问到则冲突熔断，不合成一句。开通 VPN 只调 ticket_draft 起草工单，ticket_commit 必须先收到人工 allow，对话里开通次数为 0。手册外问题直接拒绝。业务技能 policy-desk 用 SKILL.md 声明触发词，年假 / 报销 / 开通走制度值班，不再全落到站点体检。\n\n六、Trace 与评测。TraceSpan 记录 user → route → tool → reply，字段含 kind、耗时 ms、status、payload 摘要；会话写入 localStorage，刷新后可回看。Eval 用固定 query → expectedSkillId 回归路由，另有能力锁用例卡信封、结果码、误开通次数；工具侧记真实耗时。\n\n七、运行时降级。优先 Hono + SQLite；无后端时降级浏览器内运行时。IndexedDB 存长期记忆，sessionStorage 存会话态，刷新不丢、跨会话不串。\n\n技术栈：React 19、TypeScript、SSE、JSON-RPC 2.0 MCP、SKILL.md、分块 RAG、能力信封、HITL 工单、Hono、SQLite、IndexedDB。",
      performance:
        "1. 自研 Agent Loop / MCP Server / RAG / Skill 路由 / Trace / Eval，不依赖 LangChain 一类框架。\n2. SSE 增量累积 + Schema 校验 + isError 回流，流式工具调用可收敛，单点失败可继续。\n3. 路由打分和 RAG chunkId 可对账；能力信封 + 工单预演，制度问答带出处，改权限对话里误开通为 0。",
    },
  },
  {
    id: "jianchi",
